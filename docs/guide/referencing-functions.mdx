...
Running any public functions via API can be done through the [Push](/reference-v2/api/create-new-job) endpoint. Specific information for a public function can be found on the Usage Guide ([example](https://www.sievedata.com/functions/sieve/autocrop/guide)).
...
The Sieve Python client lets you run functions synchronously or asynchroously. Let’s take the [Dubbing API]("https://www.sievedata.com/functions/sieve/dubbing") for example.
