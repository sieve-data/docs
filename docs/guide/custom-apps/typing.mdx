...
Types also allow Sieve to render auto-generated UIs that can be used to run these functions via the dashboard. You may also include [Sphinx docs](https://www.sphinx-doc.org/) with each function to show descriptions with each of your inputs.
...
import sieve
audio_extractor = sieve.function.get("<org>/audio_extractor")

# Run the function with a video file
audio = audio_extractor.run(sieve.Video("path/to/video.mp4"))
print(audio.path)
```
</CodeGroup>
