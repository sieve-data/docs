...
<Note>Sieve also supports running functions with a `__setup__` method. This is especially useful for loading models into memory once for many jobs. Learn more about `sieve.Model` [here](/reference-v2/sdk/functions_models).</Note>
