...
You can create a secret by entering the name of the environment variable (e.g. `OPENAI_API_KEY`) and the value of the credential. 

![](https://storage.googleapis.com/sieve-public-data/assets/add_secret.png)

To delete a secret, you simply click the `Delete` button next to the secret and it will be removed from Sieve. To update a secret, you must first delete it and then re-create it.

![](https://storage.googleapis.com/sieve-public-data/assets/delete_secret.png)

## **Automating Secrets via API**

For those who want to programmatically set, update, and delete secrets, you can do so with the Sieve API. 
...
Finally, you can get an individual secret's information, or list all the secrets you've stored thus far. We don't return the full secret for security reasons, but we do return the first couple letters of it.
