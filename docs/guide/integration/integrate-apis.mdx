...
<Note>
  ...
  output is ready. If the function returns multiple outputs, `.result()` will
  return immediately, returning a generator object, and iterating over the
  generator will block until each output is ready.
</Note>

The job result object may or may not be a generator, depending on the function. For example, the [Lipsync API](https://www.sievedata.com/functions/sieve/lipsync) doesn't return a generator.
