...
```
 ...
 ow let’s write code that splits the video into frames and starts passing it into a face detector. In this case, we’ll use a face detection model that has already been deployed to Sieve though you could also deploy your own using a setup similar to this.
...
def vfd(video: sieve.Video):
    ...
    filenames.sort()
		...
		jobs = []
		face_detector = sieve.function.get("sieve/mediapipe-face-detector")
