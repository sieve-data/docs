...
Congrats, you just deployed a custom function to Sieve! Let's now use it to transcribe a live audio stream in Python. Make sure to replace **`<your org>`** with your actual organization name.
...
To cancel the job once you're done, you may use the `cancel` API linked [here](/reference-v2/api/jobs/cancel).

Replicas of the live transcriber automatically scale to zero when there is no traffic, so be sure to check out the [autoscaling](/guide/custom-apps/autoscaling) guide to learn more about how to configure autoscaling for your custom functions if you want to keep them running and ready to process requests.
