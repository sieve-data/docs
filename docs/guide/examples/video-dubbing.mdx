...
def video_dubbing(source_video: str, language: str):
    ...
    source_audio = sieve.Audio(path=audio_path)
    ...
    # Generate new audio
    language = "es"
    target_audio = sieve.function.get("sieve/xtts-v1").run(source_audio, language, translated_text)

    # Combine audio and video with Retalker
    retalker = sieve.function.get("sieve/video_retalking")
    return retalker.run(source_video, target_audio)

if __name__ == "__main__":
	dubbed_video = video_dubbing("./path-to-video.mp4", "es")
	print(dubbed_video)
```
...
if __name__ == "__main__":
    video = sieve.Video(path="./path-tovideo.mp4")
