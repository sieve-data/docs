...
- `persist_output` Whether the function's output should be persisted. Defaults to `False`.
...
### persist_output
```python
def persist_output(self) -> bool:
```

Whether the function's output should be persisted to permanent storage for later retrieval.
