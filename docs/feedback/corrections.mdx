...
This endpoint allows the correction of any given set of objects in a video as a way to correct mistakes made by the system. Feedback is specified in the `corrections` parameter in the same format as the payloads returned by the [/query](/query/metadata) endpoint. The system will automatically make corrections to the data specified and then improve itself automatically based on the feedback.
