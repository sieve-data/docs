...
Years have passed since the public release of models like YOLO, and we’re now at YOLOv7. Since then, thousands of companies have leveraged these models to perform the tasks that have to do with object detection, tracking, and counting to solve a variety of business problems. This includes applications in security, factories, farms, and more for tasks such as automated inventory counting, theft prevention, cattle monitoring, etc.
...
This means that you’d have to set up your own cloud or edge infrastructure to train, manage, and deploy these models — which is typically a lot of effort. Add video into the mix and it’s an almost insurmountable task for most teams. The form factor is simply too complicated and hard to setup and customizing it is possible — but difficult to do.
...
Their service is great if you’re simply looking for an app that someone on your team can use to point and click to count objects in a given setting. But there are no options if you want to set up your own stationary cameras, or if you have pre-recorded footage from something like a drone. Another downside is that they only support a fixed-set of objects. So if what you want to count isn’t supported, you’re out of luck.
...
Make queries using [Sieve](https://www.sievedata.com/)’s query endpoints (metadata, intervals)  to find the moments in a video that match a certain object count, or to pull all historical information for the sake of alerts.
