...
This endpoint allows the retrieval of a set of intervals within a video which match a query. Usage works in the same way as the [/query](/query/metadata) endpoint. The only difference is the return payload which will be a list of all intervals that match your query within each video you've submitted to your project.
